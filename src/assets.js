...
const assets = {
  ...
  make(params) {
    ...
    if (!params.shadows) {
      params.shadows = assets.buildShadows(params);
    }
 ...
 */
  ...
  buildShadows(params) {
    if (!params[settings.SHADOW]) return;
    let shadows = '';
    for (let i = 0; i < params[settings.SHADOW].length; i++) {
      shadows += assets.buildShadow(params[settings.SHADOW][i]);
    }
    return shadows;
  },

  buildShadow(params) {
    return `<div class="shadow" style="width: ${params.w}px; height: ${params.h}px; top: ${params.y}px; left: ${params.x}px;"></div>`;
  },  
  ...
  items: {
    ...
    diamond: {
      basic: {
        ...
        d: [new Collidable({ x: 0, y: 0, w: 10, h: 10 })], // a list of shadow boxes
    ...
    arch: {
      basic: {
        ...
        d: [new Collidable({ x: 0, y: 0, w: 22, h: 16 }), new Collidable({ x: 96, y: 0, w: 22, h: 16 })], // a list of shadow boxes
    ...
    bridge: {
      basic: {
        ...
        g: [new Collidable({ x: 0, y: 0, w: 0, h: 0 })], // a list of ghost collision boxes
        d: [new Collidable({ x: 0, y: 0, w: 150, h: 210 })], // a list of shadow boxes
